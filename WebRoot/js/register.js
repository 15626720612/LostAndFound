...
//					密码
					const reg2 = /((?=.*[a-z])(?=.*\d)|(?=[a-z])(?=.*[#@!~%^&*])|(?=.*\d)(?=.*[#@!~%^&*]))[a-z\d#@!~%^&*]{8,16}/i;
					var password = $("#password2").val();
					P=reg2.test(password);
					if(!P){
						alert("密码为8-16位的字母、数字或字符组合");
						return false;
					}
...
//					身份证
					const reg5 = /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/;
					var idcard = $("#idcard").val();
					IC=reg5.test(idcard);
					if(!IC){
						alert("身份证号码为15位或者18位");
						return false;
					}
					
//					邮箱
					const reg6 = /^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+/;
					var email = $("#email").val();
					EM=reg6.test(email);
					if(!EM){
						alert("请输入正确的邮箱地址");
